...
WORKER2_DNS=${worker2_dns}
VOLUME_ID=${worker2_volume_id}
...
echo "🛠️  Adding DNS record for $WORKER2_DNS -> $INSTANCE_IP"
...
# --------------------------------------------
# Register Worker2 with Citus Coordinator
# --------------------------------------------
...
if pg_isready -h "$COORDINATOR_DNS" -p "$PORT" > /dev/null 2>&1; then
    ...
    sudo PGPASSWORD="$POSTGRES_PASS" psql -h "$COORDINATOR_DNS" -U postgres  -c "SELECT master_add_node('$WORKER2_DNS', 5432);" || true
else
